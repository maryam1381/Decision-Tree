...
def print_Node(node,i):
    if node.lable=='':
        print(i*"\t"+ "Node value: "+node.value +"    Attribute: "+ node.chosen_attribute+"    Gini Index : "+str(node.gini_index)+"     info Gain : "+str(node.info_Gain))
        children = [n for n in node_list if n.parent == node]
        for child in children:
            print_Node(child,i+1)
    else:
        print(i*"\t"+"Node value: "+node.value + "    lable: "+str(node.lable)+"    Gini Index : "+str(node.gini_index))
